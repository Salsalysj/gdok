...
async function writeJsonList(list: any[]): Promise<void> {
  const dir = path.dirname(CRAFT_MATERIAL_EXCHANGES_JSON);
  await fs.mkdir(dir, { recursive: true });
  await fs.writeFile(CRAFT_MATERIAL_EXCHANGES_JSON, JSON.stringify(list, null, 2), 'utf-8');
}
...
// PUT: 제작 재료 교환 수정 (production이 아닐 때만 JSON 파일에 반영)
export async function PUT(
  ...
  if (!isCraftMaterialSaveAllowed()) {
    return NextResponse.json(
      { error: '제작 재료 교환 수정은 production 환경에서 사용할 수 없습니다.' },
      { status: 403 }
    );
  }
  try {
    ...
    console.error('제작 재료 교환 수정 실패:', e);
    return NextResponse.json(
      { error: '제작 재료 교환 수정에 실패했습니다.' },
      { status: 500 }
    );
  }
}

// DELETE: 제작 재료 교환 삭제 (production이 아닐 때만 JSON 파일에서 제거)
export async function DELETE(
  ...
  if (!isCraftMaterialSaveAllowed()) {
    return NextResponse.json(
      { error: '제작 재료 교환 삭제는 production 환경에서 사용할 수 없습니다.' },
      { status: 403 }
    );
  }
  try {
    ...
    console.error('제작 재료 교환 삭제 실패:', e);
    return NextResponse.json(
      { error: '제작 재료 교환 삭제에 실패했습니다.' },
      { status: 500 }
    );
  }
}
