...
export async function getValueDbData(): Promise<ValueDbData> {
  ...
  const entries = combinedItemList
    ...
    .sort((a, b) => {
      ...
      const growthItems = ['운명의 파괴석', '운명의 수호석', '운명의 돌파석', '운명의 파편 주머니(소)', '운명의 파편 주머니(중)', '운명의 파편 주머니(대)', '운명의 파편 1개당', '아비도스 융화 재료', '용암의 숨결', '빙하의 숨결', '베히모스의 비늘', '알키오네의 눈', '업화의 쐐기돌', '카르마의 잔영', '낙뢰의 뿔', '우뢰의 뇌옥', '고통의 가시', '은총의 파편', '유물 각인서 선택', '유물 각인서 랜덤', '젬 가공 초기화권'];
